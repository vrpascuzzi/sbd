...
/**
   ...
   @param[in] loadname: load filename for wavefunction data. if string is empty,
   use HF det as a initial state.
   @param[in] savename: save filename for wavefunction data. if string is empty,
   do not save.
   ...
   @param[out] one_p_rdm: one-particle reduced density matrix if sbd_data.do_rdm
   != 0
   @param[out] two_p_rdm: two-particle reduced density matrix if sbd_data.do_rdm
   != 0
 */
void diag(const MPI_Comm &comm, const SBD &sbd_data,
          const sbd::FCIDump &fcidump,
          const std::vector<std::vector<size_t>> &adet,
          const std::vector<std::vector<size_t>> &bdet,
          const std::string &loadname, const std::string &savename,
          double &energy, std::vector<double> &density,
  ...
  MPI_Comm b_comm;
  MPI_Comm t_comm;
  sbd::TaskCommunicator(comm, h_comm_size, adet_comm_size, bdet_comm_size,
                        task_comm_size, h_comm, b_comm, t_comm);

  auto time_start_help = std::chrono::high_resolution_clock::now();
  sbd::MakeHelpers(adet, bdet, bit_length, L, helper, sharedMemory, h_comm,
                   b_comm, t_comm, adet_comm_size, bdet_comm_size);
  sbd::RemakeHelpers(adet, bdet, bit_length, L, helper, sharedMemory, h_comm,
                     b_comm, t_comm, adet_comm_size, bdet_comm_size);
  auto time_end_help = std::chrono::high_resolution_clock::now();
  auto elapsed_help_count =
      std::chrono::duration_cast<std::chrono::microseconds>(time_end_help -
                                                            time_start_help)
          .count();
  ...
  */
  ...
  if (loadname == std::string("")) {
    ...
    sbd::LoadWavefunction(loadname, adet, bdet, adet_comm_size, bdet_comm_size,
  ...
  */
  if (method == 0) {
    ...
    */
    ...
    sbd::makeQChamDiagTerms(adet, bdet, bit_length, L, helper, I0, I1, I2, hii,
                            h_comm, b_comm, t_comm);
    sbd::Davidson(hii, W, adet, bdet, bit_length, static_cast<size_t>(L),
                  adet_comm_size, bdet_comm_size, helper, I0, I1, I2, h_comm,
                  b_comm, t_comm, max_it, max_nb, eps, max_time);
    auto time_end_davidson = std::chrono::high_resolution_clock::now();
    auto elapsed_davidson_count =
    ...
    auto elapsed_diag_count =
        std::chrono::duration_cast<std::chrono::microseconds>(time_end_diag -
                                                              time_start_diag)
            .count();
    ...
    */
    ...
    auto elapsed_mult_count =
        std::chrono::duration_cast<std::chrono::microseconds>(time_end_mult -
                                                              time_start_mult)
            .count();
    ...
    */
    ...
    sbd::makeQCham(adet, bdet, bit_length, L, helper, I0, I1, I2, hii, ih, jh,
                   hij, len, tasktype, adetshift, bdetshift, sharedInt,
                   sharedElemT, h_comm, b_comm, t_comm);
    auto time_end_mkham = std::chrono::high_resolution_clock::now();
    auto elapsed_mkham_count =
        std::chrono::duration_cast<std::chrono::microseconds>(time_end_mkham -
                                                              time_start_mkham)
            .count();
    ...
    sbd::BasisInitVector(W, adet, bdet, adet_comm_size, bdet_comm_size, h_comm,
                         b_comm, t_comm, init);
    sbd::Davidson(hii, ih, jh, hij, len, tasktype, adetshift, bdetshift,
                  adet_comm_size, bdet_comm_size, W, h_comm, b_comm, t_comm,
    ...
    */
    ...
    auto time_start_mult = std::chrono::high_resolution_clock::now();
    sbd::mult(hii, ih, jh, hij, len, tasktype, adetshift, bdetshift,
              adet_comm_size, bdet_comm_size, W, C, bit_length, h_comm, b_comm,
              t_comm);
  ...
  */
  if (do_rdm == 0) {
    ...
    sbd::OccupationDensity(oIdx, W, adet, bdet, bit_length, adet_comm_size,
                           bdet_comm_size, b_comm, res_density);
    ...
    MPI_Allreduce(density_rank.data(), density_group.data(), 2 * L, MPI_DOUBLE,
                  MPI_SUM, t_comm);
    MPI_Allreduce(density_group.data(), density.data(), 2 * L, MPI_DOUBLE,
                  MPI_SUM, h_comm);
    auto time_end_meas = std::chrono::high_resolution_clock::now();
    auto elapsed_meas_count =
        std::chrono::duration_cast<std::chrono::microseconds>(time_end_meas -
                                                              time_start_meas)
            .count();
    ...
    Correlation(W, adet, bdet, bit_length, L, adet_comm_size, bdet_comm_size,
                helper, h_comm, b_comm, t_comm, one_p_rdm, two_p_rdm);
    auto time_end_meas = std::chrono::high_resolution_clock::now();
    auto elapsed_meas_count =
        std::chrono::duration_cast<std::chrono::microseconds>(time_end_meas -
                                                              time_start_meas)
  ...
  if (ratio == 0.0) {
    ...
    sbd::CarryOverAlphaDet(W, adet, bdet, adet_comm_size, bdet_comm_size,
                           b_comm, n_kept, carryover_bitstrings,
                           truncated_weight);
    if (mpi_rank == 0) {
      std::cout << " truncated weight in carry-over = " << truncated_weight
                << std::endl;
    }
  }
 ...
 */
void diag(const MPI_Comm &comm, const SBD &sbd_data,
          const std::string &fcidumpfile, const std::string &adetfile,
          const std::string &loadname, const std::string &savename,
          double &energy, std::vector<double> &density,
          std::vector<std::vector<size_t>> &carryover_bitstrings,
          std::vector<std::vector<double>> &one_p_rdm,
  ...
  if (do_shuffle == 0) {
    ...
    if (mpi_rank == 0) {
      unsigned int seeda = static_cast<unsigned int>(
          std::chrono::high_resolution_clock::now().time_since_epoch().count());
      bdet = adet;
      sbd::ShuffleDet(adet, seeda);
      unsigned int seedb = static_cast<unsigned int>(
          std::chrono::high_resolution_clock::now().time_since_epoch().count());
